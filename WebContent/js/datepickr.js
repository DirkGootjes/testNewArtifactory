...
var datepickr = (function() {
			...
			if(e.target.className) {
				switch(e.target.className) {
					...
					case 'day':
						...
						var actualtextfiled = (this.element.id).substring(1,this.element.id.length);
						if(!document.getElementById(actualtextfiled).disabled){
							document.getElementById(actualtextfiled).value = this.element.value;
							setTextArea(this.element.value);
						}
						this.close();
					break;
				}
	...
	function setTextArea(latestVal){
		try {
    		document.getElementsByName("valueTextArea")[0].value = latestVal;
		}
		catch(err) {
		}
	}
	...
	return function(elementId, userConfig) {
		...
		this.open = function(e) {
			...
			self.calendar.style.cssText = 'display: block; position: absolute; top: '+(position.top+20)+'px; left: '+position.left+'px; z-index: 100;';
		}
